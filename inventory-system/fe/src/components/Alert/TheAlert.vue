...
<template>
  <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="transform opacity-0"
    ...
    <div v-if="show" :class="['fixed top-5 right-5 z-50 p-4 rounded-md shadow-lg', {
      ...
      <div class="flex items-center justify-between gap-3">
        ...
        <button @click="$emit('close')" class="text-xl leading-none hover:opacity-75 transition-opacity">
          ×
        </button>
      </div>
    </div>
